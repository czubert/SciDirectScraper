...
def get_pub_categories(driver):
    return driver.find_elements(By.XPATH, "(//div[@class='FacetItem'])[3]/fieldset/ol")[0]
    ...
    for option in options:
        ...
        try:
            ...
            pagination_args = [requested_num_of_publ, articles_urls,
                               driver, wait, pagination_sleep, num_of_pages_per_cat, categories]
...
def paginate(requested_num_of_publ, articles_urls, driver, wait, pagination_sleep, num_of_pages,
             categories=False, tab_opened=False):
    ...
    progress = tqdm(desc='Pagination', total=num_of_pages)
    ...
    while i < n_loops or n_loops == -1:
        ...
        # there is less publications requested than 100, so all fit in one page
        if n_loops == 1:
            return driver
